...
DOCKER_AMI=$($AWS ec2 describe-images --owners self --filters "Name=name,Values=docker*,Name=root-device-type,Values=ebs,Name=virtualization-type,Values=hvm" --query 'Images[*].[ImageId,Name,CreationDate]' --output text | sort -k 3 -r | head -1 | awk '{print $1'})
